...
const PlanetItem = ({ colors, size }) => (
  <View style={{ height, width, justifyContent: "center", alignItems: "center", backgroundColor: "black" }}>
    <Canvas style={{ width, height }}>
      <Group>
        <Circle cx={width / 2} cy={height / 2} r={size} color={colors[0]}>
          <Paint shader={RadialGradient(vec(width / 2, height / 2), size, colors)} />
        </Circle>
      </Group>
    </Canvas>
  </View>
...
const SolarSystem = () => {
  return (
    <FlatList
      ...
      renderItem={({ item }) => <PlanetItem colors={item.colors} size={item.size} />}
      pagingEnabled
